...
module.exports = client => {
  client.on("interactionCreate", async (interaction) => {
    ...
    if (!member.voice.channel)
      return interaction.reply({
        content: `<a:dinowrong:989898494313635870> **Please join a Voice Channel first!**`,
        ephemeral: true
      })
    ...
    if (player && member.voice.channel.id !== player.voiceChannel)
      return interaction.reply({
        content: `<a:dinowrong:989898494313635870> **Please join __my__ Voice Channel first! <#${player.voiceChannel}>**`,
        ephemeral: true
      })
    if (interaction.isButton()) {
      if (!player || !player.queue || !player.queue.current) {
        return interaction.reply({
          content: "<a:dinowrong:989898494313635870> Nothing Playing yet",
          ephemeral: true
        })
      }
      ...
      if (player && interaction.customId != `Lyrics` && check_if_dj(client, member, player.queue.current)) {
        return interaction.reply({
          embeds: [new MessageEmbed()
            ...
            .setTitle(`<a:dinowrong:989898494313635870> **You are not a DJ and not the Song Requester!**`)
      ...
      case "Autoplay": {
        ...
        interaction.reply({
          embeds: [new MessageEmbed()
            .setColor(es.color)
            .setTimestamp()
            .setTitle(`${player.get(`autoplay`) ? `<a:yes:833101995723194437> **Enabled Autoplay**`: `<a:dinowrong:989898494313635870> **Disabled Autoplay**`}`)
            .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({
              dynamic: true
            })))
          ]
      ...
      case "Song": {
        ...
        interaction.reply({
          embeds: [new MessageEmbed()
            .setColor(es.color)
            .setTimestamp()
            .setTitle(`${player.trackRepeat ? `<a:yes:833101995723194437> **Enabled Song Loop**`: `<a:dinowrong:989898494313635870> **Disabled Song Loop**`}`)
            .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({
              dynamic: true
            })))
          ]
      ...
      case "Queue": {
        ...
        interaction.reply({
          embeds: [new MessageEmbed()
            .setColor(es.color)
            .setTimestamp()
            .setTitle(`${player.queueRepeat ? `<a:yes:833101995723194437> **Enabled Queue Loop**`: `<a:dinowrong:989898494313635870> **Disabled Queue Loop**`}`)
            .setFooter(client.getFooter(`💢 Action by: ${member.user.tag}`, member.user.displayAvatarURL({
              dynamic: true
            })))
          ]
  ...
  client.on("messageCreate", async message => {
    ...
    if (prefixRegex.test(message.content)) {
      ...
      if (cmd || cmd.length === 0) return // message.reply("<a:dinowrong:989898494313635870> **Please use a Command Somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)})
      ...
      {
        return // message.reply("<a:dinowrong:989898494313635870> **Please use a Command Somewhere else!**").then(msg=>{setTimeout(()=>{try{msg.delete().catch(() => {});}catch(e){ }}, 3000)})
      }
    }
    ...
    if (!channel) return message.reply("<a:dinowrong:989898494313635870> **Please join a Voice Channel first!**").then(msg => {
      setTimeout(() => {
        try {
          msg.delete().catch(() => {});
        } catch (e) {}
    ...
    if (player && channel.id !== player.voiceChannel) return message.reply(`<a:dinowrong:989898494313635870> **Please join __my__ Voice Channel first! <#${player.voiceChannel}>**`).then(msg => {
      setTimeout(() => {
        try {
          msg.delete().catch(() => {});
        } catch (e) {}
...
function generateQueueEmbed(client, guildId, leave) {
  ...
  var embeds = [
    ...
    .setFooter("Powered by Dino Development")
  
      .setTitle(`Start Listening to Music, by connecting to a Voice Channel and sending either the **SONG LINK** or **SONG NAME** in this Channel!`)
      .setDescription(`> *I support <a:a_youtube:989898898636144690> Youtube, <a:spotify:984785307520090112> Spotify, <a:Soundcloud:989899023601258546> Soundcloud and direct MP3 Links!*`)
  ]
